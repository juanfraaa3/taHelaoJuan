...
export async function GET() {
  try {
    ...
    return Response.json({ records: rows.map(toClientRecord) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    return Response.json({ record: toClientRecord(record) }, { status: 201 });
  } catch (error) {
    return Response.json(
      { error: toRouteErrorMessage(error) },
      { status: 500 },
...
export async function DELETE(request: Request) {
  try {
    ...
    const db = getDb();
    await db.delete(outfitRecords).where(eq(outfitRecords.id, id));
